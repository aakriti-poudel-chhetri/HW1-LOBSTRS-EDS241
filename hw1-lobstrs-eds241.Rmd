...
**Explanation:** All three models show a positive treatment effect, meaning MPAs have more lobsters. However, the effect is only statistically significant in the poisson model (p value), not in the OLS or negative binomial models. The OLS and negative binomial models both say the difference could be due to chance.

The poisson model assumes that the spread in the data is small and predictable. When the actual data is more spread out than expected, over dispersed, the poisson model becomes overconfident and reports stronger results than it should. The negative binomial model is designed to handle this extra spread, so it gives more cautious and realistic results. The OLS model also accounts for variability more flexibly.

The three models agree that MPAs seem to have more lobsters, but two out of three models say we cannot be sure this difference is real. Therefore, the treatment effect is not reliable. Since the negative binomial and OLS models better handle the variability in data, the significant result from the poisson model is likely misleading. This shows why it is important to check assumptions and try different models before making conclusions.
...
d.  Discuss the following causal inference assumptions in the context of the MPA treatment effect estimator. Evaluate if each of the assumption are reasonable:
    ...
    In this study, SUTVA means that each site's lobster count depends only on whether that site is an MPA or not. It should not be affected by what is happening at other site. SUTVA has two conditions that must hold true for valid causal inference:
    **1. No interference:** One unit's treatment should not affect another unit's outcome. In this study, this condition is questionable because lobsters can physically move between sites. Lobsters in MPAs may migrate to nearby non-MPA sites, creating a spillover effect. This means the non-MPA sites could indirectly benefit from the protection given to MPA sites. If lobsters breed and grow in protected areas and then move to unprotected areas, the non-MPA counts would be higher than they would be without any MPAs nearby. This makes it harder to measure the true effect of protection.
    **2. No hidden variation:** The treatment should be implemented consistently across all treated units. In this study, this is not possible as all MPAs are not same. They may differ in size, how long they have been protected, or surrounding habitat quality. If some MPAs have stricter enforcement or better habitat than others, the treatment is not truly uniform across all treated sites.
    
    Because both conditions are potentially violated, SUTVA is not fully met in this study. The spillover of lobsters between sites and the inconsistent implementation of protection across MPAs make it difficult to isolate the true causal effect of MPA status on lobster abundance.
