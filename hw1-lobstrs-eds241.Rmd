...
```{r}
# Plot 1: Ridge plot - Lobster counts grouped by reef location
spiny_counts %>%
    ggplot(aes(x = counts, y = site, fill = site)) +
    geom_density_ridges(alpha = 0.6, quantile_lines = TRUE, quantiles = 2) +
    labs(title = "Lobster counts by reef location",
...
spiny_counts %>%
    ...
    labs(title = "Lobster counts by reef location",
         x = "Lobster counts",
         y = "Reefs label",
         color = "Vertical line shows median") +
    theme_minimal() +
    ...
    guides(fill = "none",
           color = guide_legend(override.aes = list(linetype = 1, 
                                                     linewidth = 1,
                                                     color = "red"))) +
    geom_vline(aes(xintercept = NA, color = "Median"), linewidth = 1)
```
...
spiny_counts %>%
    ...
    labs(title = "Lobster counts by year (2012-2018)",
         ...
         y = "Lobster counts",
         color = "") +
    scale_color_manual(values = c("Individual observations" = "steelblue", 
                                   "Mean" = "red")) +
    theme_minimal() +
...
```{r}
# Density plot: Lobster size grouped by treatment
spiny_counts %>%
    ...
    labs(title = "Lobster size distribution by treatment",
         ...
         color = "Treatment",
         fill = "Treatment") +
    scale_color_manual(values = c("0" = "#CD853F", "1" = "#2E8B57"),
                       labels = c("0" = "non-MPA", "1" = "MPA")) +
    scale_fill_manual(values = c("0" = "#CD853F", "1" = "#2E8B57"),
                      labels = c("0" = "non-MPA", "1" = "MPA")) +
    theme_minimal() +
    theme(legend.position = "bottom")
```
...
means_trt_group <- spiny_counts %>%
  gtsummary::tbl_summary(by = mpa,
    ...
    modify_spanning_header(c("stat_1", "stat_2") ~ "**Reef location**")
...
**Expanation:** This plot shows that the homogeneity of variance assumption of OLS regression is violated. Instead of forming a straight, horizontal line, the residuals display a curved, non-linear pattern. This means the spread of the residuals changes across fitted values, indicating the presence of heteroscedasticity rather than constant variance.
